...
Historical prices are either scraped via AlphaVantage or downloadeded from Yahoo!Finance as .xlsx or.json and imported into R
...
Subset the data to have same start and end point
...
gplots::heatmap.2(cor_matrix[-1,],Rowv = NA, Colv = NA, col= gplots::colorpanel(5,low="red",mid="yellow",high="green"),
                  dendrogram = "none", breaks=c(-0.1,0,0.15,0.3,0.5,1),
                  trace="none", lhei = c(0,8),lwid=c(0,4), main = "Correlogram of the SP500",
...
window.length <- 500
horizon <- 10
trControl <- trainControl(method = "timeslice", initialWindow =
...
This plot the LogReturns of the SP500 with the predicted SP500 returns from the ML model.

```{r echo=FALSE, include=TRUE}
PredObs <- function(x){
  PredObs <- cbind(xts(x["pred"], order.by = index(stocks[(window.length+1):(nrow(stocks)-(nrow(stocks)-window.length)%%horizon),])), stocks[(window.length+1):(nrow(stocks)-(nrow(stocks)-window.length)%%horizon),"sp500"])
...
```{r echo=FALSE, include=TRUE}
pick <- function(x){
  return (as.data.frame(x[c("RMSE","Rsquared","LowRMSE","HighRMSE","Accuracy","Kappa"),], make.name=T))
}

SummaryTable <- data.frame(pick(svmLinear)[1,],
...
X <-as.xts(subset(stocks, select= -c(sp500,spclass)))
Y_regr <- as.xts(subset(stocks, select= sp500))
Y_class <- as.xts(subset(stocks, select= spclass))
...
plot(performance(prediction(ClassProbcforest[,1],stocks$spclass[c(501:4900)]),"tpr","fpr"),col=rainbow(7)[7], add = T)
plot(performance(prediction(ClassProbrpart2[,1],stocks$spclass[c(501:4900)]),"tpr","fpr"),col=rainbow(7)[3], add = T)
plot(performance(prediction(ClassProbxgbLinear,stocks$spclass[c(501:4900)]),"tpr","fpr"),col=rainbow(7)[4], add = T)
plot(performance(prediction(outputProba_GRU_relu,stocks$spclass[c(501:4900)]),"tpr","fpr"),col=rainbow(7)[5], add = T)
plot(performance(prediction(outputProba_LSTM_tanh,stocks$spclass[c(501:4900)]),"tpr","fpr"),col=rainbow(7)[6], add = T)
legend(0.7, 0.7, bty="n", legend= c(paste0("SVM    ",round(performance(prediction(ClassProbsvmLinear[,1],stocks$spclass[c(501:4900)]),"auc")@y.values[[1]],digits=5)),
                                    paste0("Random Forest    ",round(performance(prediction(ClassProbcforest[,1],stocks$spclass[c(501:4900)]),"auc")@y.values[[1]], digits = 5)),
                                    paste0("XGradient Boosting    ", round(performance(prediction(ClassProbxgbLinear,stocks$spclass[c(501:4900)]),"auc")@y.values[[1]], digits = 5)),
                                    paste0("Decision Tree    ", round(performance(prediction(ClassProbrpart2[,1],stocks$spclass[c(501:4900)]),"auc")@y.values[[1]], digits = 5)),
                                    paste0("GRU    ", round(performance(prediction(outputProba_GRU_relu,stocks$spclass[c(501:4900)]),"auc")@y.values[[1]], digits = 5)),
                                    paste0("LSTM    ", round(performance(prediction(outputProba_LSTM_tanh,stocks$spclass[c(501:4900)]),"auc")@y.values[[1]], digits = 5))),
...
par(mfrow=c(2,3))
par(mar=c(2,2,3,2))
barplot(table(ifelse(ClassProbsvmLinear[,1]>0.5,1,0)), main="Class Imbalance SVM", col="black", ylim=c(0,3000))
abline(h=2200, col="blue", lty=2)
barplot(table(ifelse(ClassProbcforest[,1]>0.5,1,0)), main="Class Imbalance RF", col=rainbow(7)[7], ylim=c(0,3000))
abline(h=2200, col="blue", lty=2)
barplot(table(ifelse(ClassProbrpart2[,1]>0.5,1,0)), main="Class Imbalance DT",col=rainbow(7)[3], ylim=c(0,3000))
abline(h=2200, col="blue", lty=2)
...
barplot(table(stocks[c(501:4900),"spclass"]), main="Class Imbalance SP500", col="red", ylim=c(0,3000))
abline(h=2200, col="blue", lty=2)

class_imbalance <- matrix(data = c(table(stocks$spclass[501:4900])/4400,                              table(ifelse(df.predicted$SVM>0,1,0))/4400,
                              table(ifelse(df.predicted$RF>0,1,0))/4400,
                              table(ifelse(df.predicted$DT>0,1,0))/4400,
                              table(ifelse(df.predicted$XGB>0,1,0))/4400,
                              table(ifelse(df.predicted$GRU>0,1,0))/4400,
                            table(ifelse(df.predicted$LSTM>0,1,0))/4400),
...
```{r echo=FALSE, include=TRUE}
tocsv <- data.frame(rbind(cbind(t(confusionMatrix(data = as.factor(ifelse(ClassProbsvmLinear[,1]>0.5,1,0)),
                                                  reference = as.factor(stocks$spclass[c(501:4900)]),
                                                  positive = "1")$overall),
                                t(confusionMatrix(data = as.factor(ifelse(ClassProbsvmLinear[,1]>0.5,1,0)),
                                                  reference = as.factor(stocks$spclass[c(501:4900)]),
                                                  positive = "1")$byClass)),
                          cbind(t(confusionMatrix(data = as.factor(ifelse(ClassProbrpart2[,1]>0.5,1,0)),
                                                  reference = as.factor(stocks$spclass[c(501:4900)]),
                                                  positive = "1")$overall),
                                t(confusionMatrix(data = as.factor(ifelse(ClassProbrpart2[,1]>0.5,1,0)),
                                                  reference = as.factor(stocks$spclass[c(501:4900)]),
                                                  positive = "1")$byClass)),
                          cbind(t(confusionMatrix(data = as.factor(ifelse(ClassProbrf[,1]>0.5,1,0)),
                                                  reference = as.factor(stocks$spclass[c(501:4900)]),
                                                  positive = "1")$overall),
                                t(confusionMatrix(data = as.factor(ifelse(ClassProbrf[,1]>0.5,1,0)),
                                                  reference = as.factor(stocks$spclass[c(501:4900)]),
                                                  positive = "1")$byClass)),
...
lines(xts(1434.33*exp(cumsum(df.predicted$SVM)),index(stock_df[501:4900])), col="red", lty=3, lwd=2)
lines(xts(1434.33*exp(cumsum(df.predicted$RF)),index(stock_df[501:4900])), col=rainbow(7)[7], lty=3, lwd=2)
lines(xts(1434.33*exp(cumsum(df.predicted$DT)),index(stock_df[501:4900])), col=rainbow(7)[3], lty=3, lwd=2)
lines(xts(1434.33*exp(cumsum(df.predicted$XGB)),index(stock_df[501:4900])), col=rainbow(7)[4], lty=3, lwd=2)
lines(xts(1434.33*exp(cumsum(df.predicted$GRU)),index(stock_df[501:4900])), col=rainbow(7)[5], lty=3, lwd=2)
lines(xts(1434.33*exp(cumsum(df.predicted$LSTM)),index(stock_df[501:4900])), col=rainbow(7)[6], lty=3, lwd=2)
addLegend("topleft",
